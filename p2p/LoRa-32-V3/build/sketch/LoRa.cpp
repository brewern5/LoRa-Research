#line 1 "C:\\Users\\bobjo\\OneDrive\\Documents\\Arduino\\LoRa-32-V3\\LoRa.cpp"
#include "LoRa.h"
...
bool LoRaManager::init() {
  ...
  // Generate a random session ID on boot
  _session_id = (uint16_t)random(0xFFFF);
  Serial.printf("[LoRa] Init OK, session=0x%04X\n", _session_id);
  return true;
}
...
int sendPacket(const LoRaAudioPacket* pkt, size_t payload_len) {
  ...
  return radio.transmit(reinterpret_cast<const uint8_t*>(pkt), total_len);
}
...
bool sendAudioData(const uint8_t* data, uint8_t len) {
  ...
  int state = radio.transmit(buf, LORA_HEADER_SIZE + len);
...
bool waitForAck(uint16_t expected_seq, uint32_t timeout_ms = 2000) {
  ...
  if (radio.getPacketLength() < LORA_HEADER_SIZE + sizeof(AckPayload)) {
  ...
  Serial.printf("[RX] ACK OK for seq=%u  RSSI=%.1f  SNR=%.1f\n",
                ack.ack_seq, radio.getRSSI(), radio.getSNR());
  return true;
}